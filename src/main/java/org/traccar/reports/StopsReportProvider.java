...
public class StopsReportProvider {
    ...
    public Collection<StopReportItem> getObjects(
        ...
        for (Device device : DeviceUtil.getAccessibleDevices(storage, deviceGroupQuery.getUserId(), deviceGroupQuery.getDeviceIds(), deviceGroupQuery.getGroupIds())) {
            result.addAll(reportUtils.detectTripsAndStops(device, deviceGroupQuery.getFrom(), deviceGroupQuery.getTo(), StopReportItem.class));
        }
        return result;
    }
    ...
    public void getExcel(
        ...
        for (Device device : DeviceUtil.getAccessibleDevices(storage, deviceGroupQuery.getUserId(), deviceGroupQuery.getDeviceIds(), deviceGroupQuery.getGroupIds())) {
            Collection<StopReportItem> stops = reportUtils.detectTripsAndStops(device, deviceGroupQuery.getFrom(), deviceGroupQuery.getTo(), StopReportItem.class);
